...
// 1002 tracks button press/release plus drag motion. 1003 additionally tracks
// passive hover motion, which is noisy for editors and should be opt-in.
const MOUSE_ENABLE_BUTTON = '\x1b[?1002h\x1b[?1006h';
const MOUSE_ENABLE_ALL = '\x1b[?1003h\x1b[?1006h';
const MOUSE_DISABLE = '\x1b[?1002l\x1b[?1003l\x1b[?1006l';

const COMPLETE_SGR_MOUSE_RE = /\x1b\[<\d+;\d+;\d+[Mm]/g;
const LEAKED_SGR_MOUSE_RE = /(?:\x1b\[)?<\d+;\d+;\d+[Mm\[]?/g;

function isStandaloneMouseFragment(input: string): boolean {
  if (!input) return false;
  const normalized = input.replace(/\x1b/g, '');
  return /^\[?<\d+;\d+;\d+[Mm\[]?$/.test(normalized) || /^\d+;\d+;\d+[Mm\[]?$/.test(normalized);
}

export function stripMouseControlSequences(input: string = ''): string {
  const stripped = input.replace(COMPLETE_SGR_MOUSE_RE, '').replace(LEAKED_SGR_MOUSE_RE, '');
  return isStandaloneMouseFragment(stripped) ? '' : stripped;
}

export function isMouseControlInput(input: string = ''): boolean {
  if (!input) return false;
  return stripMouseControlSequences(input) !== input || isStandaloneMouseFragment(input);
}
...
export function useMouse(options: UseMouseOptions = {}): MouseState {
  const { enabled = true, tracking = 'button', onClick, onMove, onRelease } = options;
  ...
  useEffect(() => {
    ...
    process.stdout.write(tracking === 'all' ? MOUSE_ENABLE_ALL : MOUSE_ENABLE_BUTTON);
    setRawMode(true);
        ...
        if (event) {
          setState((prev) => {
            const newState = {
              ...
              lastClick:
                !event.isMotion && event.pressed ? { x: event.x, y: event.y } : prev.lastClick,
            };
            return newState;
          });
  ...
  }, [enabled, stdin, setRawMode, tracking]);
...
export function useGridMouse(options: UseGridMouseOptions): {
  ...
  const { enabled = true, gridLeft, gridTop, cellWidth, cellHeight, cols, rows, onClick } = options;
